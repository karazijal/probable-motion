...
class PositionEmbeddingSine3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        dim_t_div_2 = torch.div(dim_t, 2, rounding_mode='trunc')
        dim_t = self.temperature ** (2 * (dim_t_div_2) / self.num_pos_feats)

        dim_t_z = torch.arange((self.num_pos_feats * 2), dtype=torch.float32, device=x.device)
        dim_t_z_div_2 = torch.div(dim_t_z, 2, rounding_mode='trunc')
        dim_t_z = self.temperature ** (2 * (dim_t_z_div_2) / (self.num_pos_feats * 2))
